    ...
    <article class="lw-post">
        ...
        <div class="lw-container lw-post__content">
            ...
            <section class="lw-article-engage" id="comments" data-post-slug="{{ $insight['slug'] }}" data-likes="{{ $insight['likes'] ?? 0 }}" data-comments="{{ $insight['comments'] ?? 0 }}" data-shares="{{ $insight['shares'] ?? \App\Models\Post::publicCount($insight['slug'], 'shares') }}" data-views="{{ $insight['views'] ?? 0 }}">
                <div class="lw-talk__bar">
                    <p class="lw-talk__meta" data-ig-meta>{{ $insight['likes'] ?? 0 }} likes · {{ $insight['comments'] ?? 0 }} comments · {{ $insight['shares'] ?? 0 }} shares</p>
                    <div class="lw-ig-post__icons">
                ...
                <div class="lw-ig-comments lw-talk is-open">
                    <div class="lw-talk__head">
                        <p class="lw-eyebrow">Conversation</p>
                        <h2>Comments</h2>
                    </div>
                    <ul class="lw-ig-comments__list lw-talk__list" data-ig-comment-list>
                        @foreach (($insight['comment_list'] ?? []) as $comment)
                            @php $initial = mb_strtoupper(mb_substr($comment['name'] ?? 'G', 0, 1)); @endphp
                            <li class="lw-talk__item">
                                <span class="lw-talk__avatar" aria-hidden="true">{{ $initial }}</span>
                                <article class="lw-talk__bubble">
                                    <header class="lw-talk__who">
                    ...
                    <form class="lw-ig-comments__form lw-talk__form" data-ig-comment-form>
                        <span class="lw-talk__avatar" aria-hidden="true">You</span>
                        <label class="lw-visually-hidden" for="article-comment">Add a comment</label>
                        <textarea id="article-comment" name="comment" maxlength="800" rows="3" placeholder="Join the conversation…" required></textarea>
